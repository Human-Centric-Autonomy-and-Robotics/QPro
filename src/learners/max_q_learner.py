...
class MAXQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if self.args.central_mixer in ["ff", "atten"]:  #central_mixer: "ff"
        ...
        self.weight_path = './weights_save/'
        self.filename_pref = self.args.env_args['map_name'] + '_' + self.args.unique_token
        self.save_weights_t = 0
...
#th.bmm(w1,w2).view(chosen_action_logits_agents.shape)
        # Td-error
        ...
        if t_env < 30000:
            ...
            ws_min = th.min(ws).item()
            ws_max = th.max(ws).item()
        ...
        #     w_to_use = ws.mean().item() # Average of ws for logging
            ...
            res_ws = self._get_ws_weights(chosen_action_qvals, central_chosen_action_qvals, targets, chosen_action_logits_agents, w12s, w1q1_b1)
            # res_ws = th.where(td_error < 0, res_ws, th.zeros_like(td_error) * float(self.args.w))

            #calculate mean ::
            temp_sum = th.where(td_error < 0, res_ws, th.zeros_like(td_error))
            avg_mean = (temp_sum.sum() / th.count_nonzero(temp_sum.view(-1)))

            ws_min = th.min(temp_sum).item()
            ws_max = th.max(temp_sum).item()

            # Nomarlize to 0.5-1
            res_ws = th.clamp(res_ws, min = 0.0, max = self.args.mean_mul * avg_mean.item())
            res_ws = (res_ws - th.min(res_ws)) / (th.max(res_ws) - th.min(res_ws))  #0~1
            res_ws = res_ws * (1.0-float(self.args.w)) + float(self.args.w)

            # res_ws = th.where(td_error < 0, res_ws, th.ones_like(td_error) * float(self.args.w))

            ws = th.where(td_error < 0, res_ws, th.ones_like(td_error) * float(self.args.w))
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("ws_min", ws_min, t_env)
            self.logger.log_stat("ws_max", ws_max, t_env)
            if self.args.env == "one_step_matrix_game":
                print_matrix_status(batch, self.mixer, mac_out)


            self.save_weights_t += 1
            if t_env > 30000 and self.save_weights_t % 2 == 0:
                # self.filename_pref = self.args.env_args['map_name'] + '_' + self.args.unique_token
                filename = self.filename_pref + '_step_' + "{:07d}".format(t_env)
            # self.weight_path
                th.save(res_ws, self.weight_path + filename + '.th')
                th.save(ws, self.weight_path + 'ws_'+  filename + '.th')
                print("weights saved at  " + self.weight_path + filename + '.th')

    def _get_ws_weights(self, chosen_action_qvals, central_chosen_action_qvals, targets, chosen_action_logits_agents, w12s, w1q1_b1):
        w1, w2 = w12s
        a = th.exp(-1.0 * (chosen_action_qvals - central_chosen_action_qvals))
        b = (chosen_action_qvals - targets)
        c = (1-chosen_action_logits_agents)
        g1 = th.ones_like(w1q1_b1)
        g2 = th.exp(w1q1_b1)
        g = th.where(w1q1_b1 >= 0, g1, g2).repeat([1,w1.shape[1],1])
        d = th.bmm(g*w1, w2).view(chosen_action_logits_agents.shape)

        e = th.sum(th.div(c, d), 2).unsqueeze(2) - 1.0
        res = a * b * e * (-1.0)
        return res

    def _relu_get_ws_weights(self, chosen_action_qvals, central_chosen_action_qvals, targets, chosen_action_logits_agents, w12s, w1q1_b1):
        w1, w2 = w12s
        a = th.exp(-1.0 * th.abs(chosen_action_qvals - central_chosen_action_qvals))
        b = th.abs(chosen_action_qvals - targets)
        c = (2-chosen_action_logits_agents)
        g1 = th.ones_like(w1q1_b1)
        g2 = th.zeros_like(w1q1_b1)
        g = th.where(w1q1_b1 >= 0, g1, g2).repeat([1,w1.shape[1],1])
        d = th.bmm(g*w1, w2).view(chosen_action_logits_agents.shape)

        e = th.sum(th.div(c, d), 2).unsqueeze(2)
        res = a * b * e
        return res
